...
public class CS400Graph<T> implements GraphADT<T> {
	...
	public boolean removeEdge(T source, T target) {
		...
		Vertex targetVertex = this.vertices.get(target);
		if (sourceVertex == null || targetVertex == null)
			throw new IllegalArgumentException("Cannot remove edge with vertices that do not "
					+ "exist");
		// find edge to remove
		Edge removeEdge = null;
		for (Edge e : sourceVertex.edgesLeaving)
	...
	public int getWeight(T source, T target) {
		...
		Vertex targetVertex = vertices.get(target);
		if (sourceVertex == null || targetVertex == null)
			throw new IllegalArgumentException("Cannot retrieve weight of edge between vertices "
					+ "that do not exist");
		for (Edge e : sourceVertex.edgesLeaving)
			if (e.target == targetVertex)
				return e.weight;
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path copyPath, Edge extendBy) {
			// making a copy of the path by storing its start, distance, ordered sequence,
			// and end
			this.start = copyPath.start;
			this.distance = copyPath.distance;
			this.dataSequence = (List<T>) (((LinkedList<T>) copyPath.dataSequence).clone());
			this.end = copyPath.start;

			// getting the edge weight and adding it to the distance of the path to be
			// copied
			int edgeWeight = extendBy.weight;
			this.distance += edgeWeight;

			// making a copy of the edge's target vertex and adding its data into the
			// ordered sequence of data of vertices
			Vertex copyVertex = extendBy.target;
			this.dataSequence.add(copyVertex.data);
	...
	protected Path dijkstrasShortestPath(T start, T end) {
		// if no vertex containing start or end can be found, a NoSuchElementException
		// is thrown
		if ((!(vertices.containsKey(start))) || (!(vertices.containsKey(end)))) {
			...
			// a new path consisting of the passed start is created and stored in a newly
			// created Priority Queue
			Path pathShortest = new Path((vertices.get(start)));
			PriorityQueue<Path> pathNew = new PriorityQueue<Path>();
			pathNew.add(pathShortest);

			// a HashTable storing data present in a vertex and Path is created in which the
			// passed start and the path consisting of the passed start is stored
			Hashtable<T, Path> pathFinal = new Hashtable<T, Path>();
			pathFinal.put(start, pathShortest);

			// we keep iterating till the created Priority Queue is not empty
			while ((!(pathNew.isEmpty()))) {
				// we remove a path from the Priority Queue and store it so as to iterate over
				// it using an iterator
				pathShortest = pathNew.remove();
				Iterator<Edge> pathIterator = pathShortest.end.edgesLeaving.iterator();

				// we keep iterating till we have edges to iterate over
				while (pathIterator.hasNext()) {
					// We retrieve an edge from the path we are iterating over and temporarily
					// store it along with the path we are iterating over
					Edge pathEdge = pathIterator.next();
					pathTemporary = new Path(pathShortest, pathEdge);

					// we check if the HashTable has the retrieved edge's target's data value
					// stored as a key
					if (pathFinal.containsKey(pathEdge.target.data)) {

						// we check if the Path stored in the HashTable with the key of the
						// retrieved edge's target's data value is larger than the temporarily
						// stored path
						if (((pathFinal.get(pathEdge.target.data)).compareTo(pathTemporary)) > 0) {
							// we store the temporarily stored path in the HashTable with the key
							// of the retrieved edge's target's data value and also add it to the
							// Priority Queue
							pathFinal.put(pathEdge.target.data, pathTemporary);
							pathNew.add(pathTemporary);
						} else {
							continue;
						}
					} else {
			...
			// we check if there currently does not exist a path in the HashTable with the
			// key end
			if ((pathFinal.get(end)) == null) {
				throw new NoSuchElementException();
			} else {
				return pathFinal.get(end);
			}
		}
	}
